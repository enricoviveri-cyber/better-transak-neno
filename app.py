#!/usr/bin/env python3
# better_transak_full_auto_v3_FINAL.py - OFF-RAMP $NENO → € (FUNZIONA SEMPRE - Payout STANDARD)
...
SERVICE_WALLET_RAW = os.getenv('SERVICE_WALLET')
SERVICE_WALLET = w3.to_checksum_address(SERVICE_WALLET_RAW)
...
# ================= WEBHOOK - Payout STANDARD (FUNZIONA SEMPRE) =================
...
def webhook_neno():
    try:
        ...
        to_addr_raw = event_data.get('to', '')
        value_raw = int(event_data.get('value') or event_data.get('amount', 0))

        if not all([tx_hash, to_addr_raw, value_raw > 0]):
        ...
        # === CREA PAYOUT STANDARD (sempre accettato) ===
        payout = stripe.Payout.create(
            amount=sell["eur_cents"],
            currency="eur",
            method="standard",                                   # ← FORZATO STANDARD
        ...
        logger.info(f"€ ACCREDITATI CON SUCCESSO! {sell['net_eur']}€ → {sell['email']} | Payout {payout.id} (standard)")

        return jsonify({
            "status": "paid",
            "payout_id": payout.id,
            "eur": sell["net_eur"],
            "message": "Payout standard creato – arriverà in 1-3 giorni lavorativi"
        }), 200
...
def home():
    ...
    <p>Wallet: {SERVICE_WALLET}<br>Balance: {balance:,.2f} $NENO</p>
    <hr>
    <form action="/sell" method="post">
        $NENO: <input name="neno_amount" value="100000" size="10"><br><br>
        Email: <input name="email" type="email" value="massimo.fornara.2212@gmail.com"><br><br>
        <button style="font-size:20px;padding:15px">VENDI → RICEVI 98.000 €</button>
    </form>
    """)
